...
export const createAdmin = asyncHandler(async (req, res, next) => {
    ...
    if (existingAdmin)
        return next(new ErrorResponse("Admin already exists", statusCode.Bad_Request));
...
export const loginAdmin = asyncHandler(async (req, res, next) => {
    ...
    if (!email || !password)
        return next(new ErrorResponse("Email and password are required", statusCode.Bad_Request));
    const admin = await Admin.findOne({ email });
    if (!admin)
        return next(new ErrorResponse("Admin not found", statusCode.Not_Found));
    const isPasswordValid = await verifyPassword(password, admin.password);
    if (!isPasswordValid)
        return next(new ErrorResponse("Invalid password", statusCode.Bad_Request));
...
export const getAdminById = asyncHandler(async (req, res, next) => {
    ...
    if (!id)
        return next(new ErrorResponse("Admin id is required", statusCode.Bad_Request));
