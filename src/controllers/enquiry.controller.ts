...
export const getEnquiryById = asyncHandler(async (req, res, next) => {
    ...
    if(!id) return next(new ErrorResponse("Enquiry id is required", statusCode.Bad_Request))
    ...
    if(!enquiry) return next(new ErrorResponse("Enquiry not found", statusCode.Not_Found))
