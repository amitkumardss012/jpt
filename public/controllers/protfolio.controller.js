...
export const createPortfolio = asyncHandler(async (req, res, next) => {
    ...
    if (!image)
        return next(new ErrorResponse("Please upload an image", statusCode.Bad_Request));
...
export const getPortfolioById = asyncHandler(async (req, res, next) => {
    ...
    if (!portfolio)
        return next(new ErrorResponse("Portfolio not found", statusCode.Not_Found));
    return res.status(statusCode.OK).json({
        message: "Portfolio fetched successfully",
        data: portfolio
    });
...
export const deletePortfolio = asyncHandler(async (req, res, next) => {
    ...
    if (!id)
        return next(new ErrorResponse("Portfolio id is required", statusCode.Bad_Request));
...
//# sourceMappingURL=protfolio.controller.js.map